...
interface BluetoothDevice {
  ...
  // Add other properties like event listeners if needed
...
interface BluetoothRemoteGATTCharacteristic {
  ...
  // Add other methods like writeValueWithResponse, writeValueWithoutResponse if needed
  ...
  removeEventListener(type: 'characteristicvaluechanged', listener: (this: this, ev: Event) => any, options?: boolean | EventListenerOptions): void;
}
...
// REMOVED: interface EventListenerOrEventListenerObject {
// REMOVED:   handleEvent(evt: Event): void;
// REMOVED: }


// --- React 18 Root API type fix ---
interface Root {
...
// --- Data for Juz Amma (Surahs 78-114) ---

// --- Components ---

// Add type definitions for Surah and Verse
interface Verse {
...
const SurahSelectionScreen = ({ surahs, onSelectSurah, onToggleBluetooth, bluetoothStatus, connectedDeviceName, autoReconnectEnabled, onToggleAutoReconnect }: {
  ...
  return (
    React.createElement('div', { className: 'container' },
      React.createElement('h1', { className: 'header', "aria-live": "polite" }, 'Select a Surah from Juz Amma'),
      React.createElement('ul', { className: 'surah-list', role: 'listbox' },
        surahs.map((surah) => (
          React.createElement('li', {
            key: surah.number,
            className: 'surah-list-item',
            onClick: () => onSelectSurah(surah),
            onKeyPress: (e) => { if (e.key === 'Enter' || e.key === ' ') onSelectSurah(surah); },
            tabIndex: 0,
            role: 'option',
      ...
      React.createElement('div', { className: 'bluetooth-controls', style: { marginTop: '20px', paddingBottom: '20px', textAlign: 'center' } },
         ...
         }, connectedDeviceName ? `Disconnect ${connectedDeviceName}` : 'Connect Bluetooth Remote'),
         React.createElement('p', { className: 'bluetooth-status', 'aria-live': 'polite', style: { marginTop: '10px', fontSize: '0.9em', color: '#555' } }, bluetoothStatus),
...
const VerseDisplayScreen = ({ surah, verseIndex, onNext, onPrev, onBack }: {
  ...
  return (
    React.createElement('div', { className: 'verse-display landscape-optimized' },
      React.createElement('div', { className: 'verse-header-row' },
        React.createElement('div', { className: 'verse-title-block' },
          React.createElement('h2', { className: 'surah-title', 'aria-live': 'polite' }, `${surah.englishName} `,
            React.createElement('span', { className: 'surah-arabic' }, `(${surah.name})`)),
          React.createElement('p', { className: 'verse-info', 'aria-live': 'polite' }, `Verse ${currentVerse.numberInSurah} of ${totalVerses}`)
        )
      ),
      React.createElement('div', { className: 'verse-main-row' },
        React.createElement('div', { className: 'verse-arabic-block' },
      ...
      React.createElement('div', { className: 'verse-translation-block' },
        React.createElement('p', {
      ...
      React.createElement('div', { className: 'verse-footer-nav' },
        React.createElement('button', {
          className: 'nav-button',
          onClick: onPrev,
...
const App = () => {
  const [currentScreen, setCurrentScreen] = useState<'surahList' | 'verseDisplay'>('surahList');
  ...
  let keepAliveCharacteristic: BluetoothRemoteGATTCharacteristic | null = null;
...
const attemptReconnect = async (deviceParam?: BluetoothDevice): Promise<void> => {
  ...
  const connectBluetoothDevice = async (): Promise<void> => {
    ...
    try {
      ...
      notifyCharacteristic.addEventListener('characteristicvaluechanged', handleBluetoothData as EventListener);
      ...
      device.addEventListener('gattserverdisconnected', () => {
        ...
        if (notifyCharacteristic) {
          notifyCharacteristic.removeEventListener('characteristicvaluechanged', handleBluetoothData as EventListener);
        }
      ...
      if (error instanceof DOMException && error.name === 'NotFoundError') {
        ...
        setBluetoothStatus(`Error: ${(error as Error).message}`);
      }
      setConnectedBluetoothDevice(null);
      setControlCharacteristic(null);
    }
  ...
  const onZikrButtonPress = (counter: number, isDoubleClick = false) => {
  ...
  const disconnectBluetoothDevice = async () => {
    ...
    if (!connectedBluetoothDevice || !connectedBluetoothDevice.gatt) return;
    ...
    try {
      if (controlCharacteristic) {
        ...
        controlCharacteristic.removeEventListener('characteristicvaluechanged', handleBluetoothData as EventListener);
        setControlCharacteristic(null);
      }
      if (connectedBluetoothDevice.gatt.connected) {
      ...
      setBluetoothStatus(`Disconnection error: ${(error as Error).message}`);
    } finally {
      setConnectedBluetoothDevice(null);
    }
  };
  ...
  let notifyCharacteristic: BluetoothRemoteGATTCharacteristic | null = null; // hoist for disconnect event
  const establishServiceConnection = async (device: BluetoothDevice): Promise<void> => {
    if (!device.gatt) {
  ...
  return (
    <>
      ...
      {currentScreen === 'verseDisplay' && selectedSurah ? (
        React.createElement(VerseDisplayScreen, {
          surah: selectedSurah,
...
const reactRoot = (ReactDOM as unknown as ReactDOMClient).createRoot(rootElement!); 
